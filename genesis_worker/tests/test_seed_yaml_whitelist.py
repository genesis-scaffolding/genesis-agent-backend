...
from genesis_worker.utils.services.seed_yaml_whitelist import (
...
def test_seed_config_writes_when_absent(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "genesis_worker.utils.services.seed_yaml_whitelist._host_connected_subnets",
        list,
    )
    monkeypatch.setattr(
        "genesis_worker.utils.services.seed_yaml_whitelist._host_own_addresses",
        list,
    )
    assert seed_yaml_whitelist(tmp_path / "config.yaml", key="whitelist") is True
...
def test_seed_config_corrects_preexisting_default(
    ...
    monkeypatch.setattr(
        "genesis_worker.utils.services.seed_yaml_whitelist._bridge_gateways", lambda: ["172.18.0.1"]
    )
    monkeypatch.setattr(
        "genesis_worker.utils.services.seed_yaml_whitelist._host_connected_subnets",
        list,
...
def test_seed_config_falls_back_without_docker(
    ...
    monkeypatch.setattr("genesis_worker.utils.services.seed_yaml_whitelist._bridge_gateways", list)
    monkeypatch.setattr(
        "genesis_worker.utils.services.seed_yaml_whitelist._host_connected_subnets",
        list,
    )
...
def test_seed_config_is_noop_when_already_correct(
    ...
    assert seed_yaml_whitelist(tmp_path / "config.yaml", key="whitelist") is False
...
def test_seed_config_with_custom_key(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert seed_yaml_whitelist(tmp_path / "config.yaml", key="allowedHosts") is True
    data = _load(tmp_path)
...
def test_seed_config_uses_extras_when_provided(
    ...
    assert seed_yaml_whitelist(
        tmp_path / "config.yaml", key="whitelist", extras=["10.0.0.0/8", "203.0.113.7"]
    )
    data = _load(tmp_path)
...
def test_seed_config_disable_docker_hosts_false_keeps_field(
    ...
    assert (
        seed_yaml_whitelist(tmp_path / "config.yaml", key="whitelist", disable_docker_hosts=False)
        is True
    )
    data = _load(tmp_path)
...
def test_host_own_addresses_parses_interfaces(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "genesis_worker.utils.services.seed_yaml_whitelist.subprocess.run",
