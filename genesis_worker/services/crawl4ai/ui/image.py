"""Image management — install / uninstall / version picker for the Crawl4AI Docker image.

Mirrors ``genesis_worker/services/sillytavern/ui/image.py``: one
installable per upstream image, active selection in
``<state_dir>/crawl4ai/current``, pinning via the dropdown below. Inline
install progress is shown during ``docker pull`` via a
refresh-on-fragment widget.
"""
...
st.caption(
    ...
    "``<state_dir>/crawl4ai/current``; pinning is via the dropdown below."
)
