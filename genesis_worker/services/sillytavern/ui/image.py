"""Image management — install / uninstall / version picker for the SillyTavern Docker image.

Mirrors ``genesis_worker/services/comfyui/ui/image.py``: one installable
per upstream image, active selection in ``<state_dir>/sillytavern/current``,
pinning via the dropdown below. Inline install progress is shown during
``docker pull`` via a refresh-on-fragment widget.
"""
...
st.caption(
    ...
    "``<state_dir>/sillytavern/current``; pinning is via the dropdown below."
)
