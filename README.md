...
| `bin/build-config.py` | reads the catalog + `recipes.yaml`, emits `config.generated.yaml` (one llama-swap entry per model) |
...
# 4. Build catalog + config
make all

# 5. Swap in the generated config and start llama-swap
...
5. `make config`, then `mv config.generated.yaml config.yaml && make up`
...
├── config.generated.yaml          generated by make config
├── bin/
│   ├── catalog.py
│   ├── build-config.py
│   ├── up
